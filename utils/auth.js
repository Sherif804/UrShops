...
let isSharedAuthorized = async (req, res, next) => {
    try {
        let response;
        if (req.headers.authorization) {
            let bareToken = req.headers.authorization;
            ...
            req.role = decoded.split(process.env.SECRET_AUTH_KEY)[2];
            req.id = decoded.split(process.env.SECRET_AUTH_KEY)[1];
            if( ( req.role == "admin" || req.role == "superAdmin" || req.role == "user") && decodedArrayLength == 4 && lastDecodedElement == process.env.SECRET_HASH_KEY) {
                next(req,res);
            }
            else {
                response = { status: 401, message: "unauthorized" };
